...
class Tapper:
    def __init__(self, tg_client: Client, multi_thread: bool):
        ...
        self.my_ref = "frndId6142904262"
        self.ref = "frndId6142904262"
                ...
                for card in cards:
                    if card['boughtAt'] is not None:
                        if int(time_module.time())*1000 < card['boughtAt'] + card['rechargingDuration']:
                            continue
                    if card['condition'] is None and int(self.balance) >= int(card['nextLevel']['cost']):
                        available_cards.append(card) 
                    else:
                        continue
                    try:
                        if card['condition']['passed'] and int(self.balance) >= int(card['nextLevel']['cost']):
                            available_cards.append(card)
